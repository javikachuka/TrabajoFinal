                ...
                <tbody>
                    @foreach($horarios as $horario)
                        ...
                        <tr>
                            ...
                            <td width ="150px">
                                @can('horarios_edit')
                                    <a href=""  class="btn btn-secondary btn-xs " data-toggle="modal" data-target="#editar{{$horario->id}}" >Editar</a>

                                    <!-- Modal Edit -->
                                    <div class="modal fade" id="editar{{$horario->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                        ...
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                    <div class="form-group">
                                                            <label>Nombre</label>
                                                            <input type="text" name="nombre" required value="{{ $horario->nombre ?? old('nombre')}}"  class="form-control">
                                                    </div>

                                                    <div class="form-group">
                                                            <label for="">Hora de Entrada</label>
                                                            <input type="time" class="form-control"  name="horaEntrada" value="{{ $horario->horaEntrada ?? old('horaEntrada')}}" required>
                                                      </div>

                                                      <div class="form-group">
                                                            <label for="    ">Hora de Salida</label>
                                                            <input type="time" class="form-control"  name="horaSalida" value="{{ $horario->horaSalida ?? old('horaSalida')}}"  required>
                                                      </div>
                                ...
                                @endcan
                            <form method="POST" action="horarios/{{$horario->id}}" onsubmit="return confirm('Desea borrar {{$horario->nombre}}')" style="display: inline-block;">
                                    @csrf
                                    @method('DELETE')
