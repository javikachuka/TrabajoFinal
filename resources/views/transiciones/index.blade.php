            ...
            <table id="users" class="table table-sm table-bordered table-striped table-hover datatable">
                ...
                <tbody>
                    @foreach ($flujoTrabajo->transiciones as $tran)
                        ...
                        <td width="75px">
                            <form method="POST" action="{{route('transiciones.destroy', $tran->id)}}" onsubmit="return confirm('Desea borrar a {{$tran->nombre}}')" style="display: inline-block;">
                                @method('DELETE')
                                @csrf
                                    @can('transiciones_destroy')
                                        <button type="submit" class="btn btn-sm btn-danger btn-xs btn-delete"><i class="fas fa-minus"></i></button>
                                    @endcan
                            </form>
                        </td>
                        </tr>
